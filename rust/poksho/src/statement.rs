...
impl Statement {
    ...
    pub fn prove(
        ...
        for point in &commitment {
            sho.absorb(&point.compress().to_bytes());
            //println!("p_point: {:?}", &point.compress().to_bytes());
        }
    ...
    pub fn verify_proof(
        ...
        // commitment的第二个和prove对不上, 最后发现是因为credential那里写错了:(
        for point in &commitment {
            // R
            sho.absorb(&point.compress().to_bytes());
            //println!("v_point: {:?}", &point.compress().to_bytes());
        }
    ...
    fn homomorphism_with_subtraction(
        ...
        for e in &self.equations {
            ...
            // general case points might be secret (no  t just scalars!)
    ...
    fn sort_scalars(&self, scalar_args: &ScalarArgs) -> Result<G1, PokshoError> {
        if scalar_args.0.len() != self.scalar_vec.len() {
            ...
            println!("scalar_args length: {}", scalar_args.0.len());
            // 打印self中存储的标量数量
            println!("self.scalar_vec length: {}", self.scalar_vec.len());
