...
// This is the function that I need to use to get the i1 and i2 from the fingerprint
use util::{get_alt_index_from_u8, get_fai_from_u8};


/// If insertion fails, we will retry this many times.
pub const MAX_REBUCKET: u32 = 2000;
...
pub struct CuckooFilter<H> {
    ...
    len: usize,
    pub _hasher: std::marker::PhantomData<H>,
}
    ...
    H: Hasher + Default,
{
    ...
    pub fn with_capacity(cap: usize) -> Self {
        ...
        let capacity = cmp::max(cap / BUCKET_SIZE, 1);
        Self {
    ...
    pub fn contains_u8(&self, data: &[u8;FINGERPRINT_SIZE]) -> bool {
        let FaI { fp, i1, i2 } = get_fai_from_u8::<H>(&data);
        let len = self.buckets.len();
        self.buckets[i1 % len]
            .get_fingerprint_index(fp)
            .or_else(|| self.buckets[i2 % len].get_fingerprint_index(fp))
    ...
    pub fn get_i1_from_u8(buc_length: usize, data: &[u8;FINGERPRINT_SIZE]) -> usize{
        let FaI { fp, i1, i2 } = get_fai_from_u8::<H>(&data);
        i1 % buc_length
    }

    pub fn get_i2_from_u8(buc_length: usize, data: &[u8;FINGERPRINT_SIZE]) -> usize{
        let FaI { fp, i1, i2 } = get_fai_from_u8::<H>(&data);
        i2 % buc_length
    }
    ...
    pub fn add<T: ?Sized + Hash>(&mut self, data: &T) -> Result<(), CuckooError> {
        ...
        // println!("Dropped: {:?}", fp.data);
        Err(CuckooError::NotEnoughSpace)
    }


    // Caution:: In this function, we use random_index_deterministic function instead of random_index, 
    //           so same input will lead to same output. 
    pub fn add_u8(&mut self, data: &[u8;FINGERPRINT_SIZE]) -> Result<(), CuckooError> {
        let fai = get_fai_from_u8::<H>(data);
        if self.put(fai.fp, fai.i1) || self.put(fai.fp, fai.i2) {
            // for (index, i) in self.buckets.iter().enumerate(){
            //     println!("Index {}: {:?}",index, i.buffer[0].data);
            // }
            // println!();
            return Ok(());
        }
        let len = self.buckets.len();
        // let mut i = fai.random_index(&mut rng);
        let mut i = fai.random_index_deterministic(fai.i1);
        let mut fp = fai.fp;
        for _ in 0..MAX_REBUCKET {
            let other_fp;
            {
                let loc = &mut self.buckets[i % len].buffer[0];
                other_fp = *loc;
                *loc = fp;
                i = get_another_index::<H>(other_fp, i, len);
            }
            if self.put(other_fp, i) {
                // for (index, i) in self.buckets.iter().enumerate(){
                //     println!("Index {}: {:?}",index, i.buffer[0].data);
    ...
    pub fn test_and_add_u8(&mut self, data: &[u8;FINGERPRINT_SIZE])-> Result<bool, CuckooError> {
        if self.contains_u8(data) {
            Ok(false)
        } else {
            self.add_u8(data).map(|_| true)
        }
    }
...
pub fn get_another_index<H: Hasher + Default>(fp:Fingerprint, i: usize, len: usize) -> usize{
    let fai = get_fai_from_u8::<H>(&fp.data);
    let i = i % len;
    let i1 = fai.i1 % len;
    let i2 = fai.i2 % len;
    if i1 == i{
        ...
        println!("fp: {:?}, i: {}, i1: {}, i2: {}",fp.data, i, i1, i2);
        panic!("This is not the index of the fingerprint");
