...
// solve the problem that: when fingerprint is equal to data, the i2 will always be 0
pub fn get_alt_index_from_u8<H: Hasher + Default>(fp: Fingerprint, i: usize) -> usize {
    let mut hasher = <H as Default>::default();
    fp.data.hash(&mut hasher);
    i.hash(&mut hasher);
    let result = hasher.finish();
    let alt_i = i as usize;
    (result as usize ^ alt_i) as usize
}

impl FaI {
    fn from_u8_Data<H: Hasher + Default>(data: &[u8;FINGERPRINT_SIZE]) ->Self{
        let (_, index_hash) = get_hash::<_, H>(data);

        let fp: Fingerprint = Fingerprint::from_data(*data).unwrap();
        let i1 = index_hash as usize;
        let i2 = get_alt_index_from_u8::<H>(fp, i1);
        Self{
            fp,
            i1,
            i2,
    ...
    fn from_data<T: ?Sized + Hash, H: Hasher + Default>(data: &T) -> Self {
        let (fp_hash, index_hash) = get_hash::<_, H>(data.clone());
    ...
    pub fn random_index_deterministic(&self, i1: usize) -> usize{
        if i1 % 2 == 0{
            self.i1
        }
        else {
...
pub fn get_fai_from_u8<H: Hasher + Default>(data: &[u8;FINGERPRINT_SIZE]) -> FaI {
    FaI::from_u8_Data::< H>(data)
}
